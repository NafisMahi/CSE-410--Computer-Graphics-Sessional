...
public:
    ...
    Vector3D light_coefficient;
    bitmap_image blackTextureImg;
    bitmap_image whiteTextureImg;
    checkerBoard(double width_of_each_cell, Vector3D light_coefficient) {
        this->width_of_each_cell = width_of_each_cell;
        this->light_coefficient = light_coefficient;
        blackTextureImg = bitmap_image("C:/Academics/4-1/graphics_lab/offline_4/OpenGL Programming/implementation/texture_b.bmp");
        whiteTextureImg = bitmap_image("C:/Academics/4-1/graphics_lab/offline_4/OpenGL Programming/implementation/texture_w.bmp");
    }
    // draw a checkerboard on the xy plane with two for loops
    void draw() {
        int numCells = 50;
        double cellWidth = width_of_each_cell;
        glBegin(GL_QUADS);
