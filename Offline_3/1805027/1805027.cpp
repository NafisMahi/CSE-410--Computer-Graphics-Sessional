...
Color calculateColorCoefficients(Vector3D intersectionPoint, Color intersectionPointColor, Vector3D normal, Object *object,int level)
{
    ...
    if (object == NULL)
    {
        color.r = color.r + (lambert * checkerBoard_color.g) * intersectionPointColor.r;
        color.g = color.g + (lambert * checkerBoard_color.g) * intersectionPointColor.g;
        color.b = color.b + (lambert * checkerBoard_color.g) * intersectionPointColor.b;
    ...
    if (level < recursion_level)
    {
        ...
        for (int i = 0; i < objects.size(); i++)
        {
            ...
            if (t > 0 && t < min_t)
            {
                ...
                index = i;
                color_ = object->color;
                coeff = object->coEfficients[3];
            }
        }
        if(index != -1){
            normal = object->getNormal(ray.start + ray.dir * min_t);
            // Vector3D reflected = reflectedRay(normalize(intersectionPoint - pos), normal);
            // if(dot(normal, reflected) < 0)
            // {
            //     normal = normal * -1;
        ...
        if(t < min_t && t > 0)
        {
            ...
            //normal = Vector3D(0, 0, 1);
            object = NULL;
            coeff = checkerBoard_color.b;
        }

        if (index != -1)
        {
            Color colortemp(0, 0, 0);
            colortemp = calculateColorCoefficients(intersectionPoint + reflected * 0.0001 + reflected * min_t, color_, normal, object, level + 1);
            color.r += colortemp.r * coeff;
            color.b += colortemp.b * coeff;
            color.g += colortemp.g * coeff;
        }
    }
